...
def run():
    ...
    total = 5000
    data = {
        "txn_id": [f"FK-{i:09d}" for i in range(total)],
        "user_id": [f"USR-{random.randint(1, 50000)}" for _ in range(total)],
        "amount": np.round(np.random.uniform(20.0, 40000.0, total), 2),
        "payment_mode": [random.choice(['FK-PayLater', 'Credit Card', 'UPI', 'COD']) for _ in range(total)],
        "vendor": "Flipkart", # Capitalized for Data
        "txn_date": [(datetime.now() - timedelta(days=random.randint(0, 2))).strftime('%Y-%m-%d %H:%M:%S') for _ in range(total)]
    }
